...
export default async function TradeDetailPage({ params }: TradeDetailPageProps) {
  ...
  try {
    ...
    const insight =
      insights.find((report) => report.scope === "trade" && report.scopeKey === trade.id) ??
      buildTradeInsight(trade, metrics);
    ...
    return (
      <WorkspaceShell
        currentPath="/trades"
        title="Trade review"
        description="This trade page is temporarily unavailable because the data connection did not respond."
      >
