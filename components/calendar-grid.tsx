...
export function CalendarGrid({
  ...
  return (
    <div className="space-y-3">
      <div className={cn("pb-1", preview ? "overflow-visible" : "overflow-x-auto")}>
        <div className={cn("space-y-3 px-2", preview ? "min-w-0" : "min-w-[760px] sm:px-1")}>
          <div className={cn("grid", preview ? "gap-1.5" : "gap-2")} style={columnStyle}>
            {weekdayHeaders.map((day) => (
              <div
                key={day}
                className="px-2 py-1 text-center font-mono text-[11px] uppercase tracking-[0.18em] text-muted"
              >
                {day}
              </div>
            ))}
                ...
                {week.days.map((day) => (
                  <button
                    ...
                    className={cn(
                      ...
                      preview ? "min-h-[106px] rounded-[1rem] px-2.5 py-2.5" : "min-h-[124px] rounded-[1.2rem] px-3 py-3",
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span
                        className={cn(
                          "max-w-[5.25rem] text-right font-semibold leading-tight",
                          day.topRight.length > 8 ? "text-[10px]" : "text-[11px]",
                    ...
                    <div className="flex items-center justify-center py-2 text-center">
                      <div
                        className={cn(
                          ...
                          preview ? "text-[15px]" : "text-[15px]"
                        )}
                      >
                        {day.body}
                      </div>
                    </div>
                    <div className={cn("text-center leading-none text-muted", preview ? "text-[10px]" : "text-[11px]")}>
                      {day.footer}
                    </div>
                  </button>
                ))}
